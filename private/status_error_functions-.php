...
function require_login() {
  ...
  } elseif ($session->is_logged_in() && !empty($session->technician_id)) {
    redirect_to(url_for('/index.php')); //to redirects technicians from admin pages
  } elseif ($session->is_logged_in() && !empty($session->customer_id)) {
...
function display_errors($errors=array()) {
  ...
  if(!empty($errors)) {
    ...
    $output .= "<ul style='list-style-type:none;font-size:13px;'>";
...
function require_login_technician() {
  global $session;
  if(!$session->is_logged_in()) {
    redirect_to(url_for('/technicians/login.php'));
  } elseif ($session->is_logged_in() && empty($session->technician_id)) {
    redirect_to(url_for('/technicians/login.php')); //to redirects other users from riders pages
  }
}
